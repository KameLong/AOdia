...
public class StationInfoFragment extends AOdiaFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        try {
            diaFile = ((AOdiaActivity) getActivity()).getDiaFiles().get(fileNum);
        }catch(Exception e){
            SdLog.log(e);
        ...
        usedTrainType=new boolean[diaFile.getTrainTypeNum()];
        for(int i=0;i<subName.length;i++){
            subName[i]="";
            subNameCount[i]=0;
        }
        for(int i=0;i<diaFile.getTrainTypeNum();i++){
    ...
    private void makeTimeTable(){
        ...
        for(int i=trainList.size()-1;i>=0;i--){
            ...
            int hour=train.predictTime(station,0)/3600;
            if(hour<3||hour>26){
                hour=hour+21;
                hour=hour%24;
                hour=hour+3;
            }
            String min=""+(train.predictTime(station,0)/60)%60;
            int color=diaFile.getTrainType(train.getType()).getTextColor().getAndroidColor();
            if(train.getStopType(station)!=AOdiaTrain.Companion.getSTOP_TYPE_STOP()){
                color=Color.rgb(150,150,150);
            }
            if(!train.existTime(station)){
                min=min+"?";
            }
            String destination=diaFile.getStation(train.getEndStation()).getName();
            String addInfo="";
            ...
            if(train.getEndStation()+(1-2*direct)>=0&&train.getEndStation()+(1-2*direct)<diaFile.getStationNum()){
                if(train.getStopType(train.getEndStation()+(1-2*direct))==AOdiaTrain.Companion.getSTOP_TYPE_PASS()
                        ||train.getStopType(train.getEndStation()+(1-2*direct))==AOdiaTrain.Companion.getSTOP_TYPE_NOVIA()){
                    addInfo+="||";
                    existGoOther=true;
                }
            }

            StationTimeTableTrainView trainView=new StationTimeTableTrainView(getActivity(),""+min,subName[train.getEndStation()],addInfo,color,fileNum,diaNumber,direct,trainList.get(i));
        ...
        for(int i=0;i<usedTrainType.length;i++){
            ...
            if(usedTrainType[i]){
                ...
                typeView.setTextColor(diaFile.getTrainType(i).getTextColor().getAndroidColor());
...
//            StationTimeTableTrainView exampleView=new StationTimeTableTrainView(getActivity(),"00:通過列車","","",Color.rgb(150,150,150),fileNum,diaNumber,direct,-1);
    ...
    private ArrayList<Integer> makeTrainArray(){
        ...
        for (int i = 0; i < diaFile.getTrainNum(diaNumber, direct); i++) {
            if (diaFile.getTrain(diaNumber, direct, i).predictTime(station,0)>0){
                if(diaFile.getTrain(diaNumber, direct, i).getStopType(station)!=AOdiaTrain.Companion.getSTOP_TYPE_STOP()&&!spf.getBoolean("STTpass",false)){
                    continue;
                }
                if(diaFile.getTrain(diaNumber, direct, i).getEndStation()==station&&!spf.getBoolean("endTrain",false)){
                    continue;
                }
                addStationSubName(diaFile.getTrain(diaNumber, direct, i).getEndStation());
                if(diaFile.getTrain(diaNumber, direct, i).getStopType(station)==AOdiaTrain.Companion.getSTOP_TYPE_STOP()){
                    usedTrainType[diaFile.getTrain(diaNumber, direct, i).getType()]=true;
                }
                int j = 0;
                for (j = trainList.size(); j > 0; j--) {
                    if (diaFile.getTrain(diaNumber, direct, i).predictTime(station,0) < diaFile.getTrain(diaNumber, direct, trainList.get(j - 1)).predictTime(station,0)) {
                        break;
                    }
                }
                trainList.add(j,i);
